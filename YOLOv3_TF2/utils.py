...
def non_max_suppression(inputs, model_size, max_output_size, max_output_size_per_class, iou_threshold, confidence_threshold):
    ...
    bbox = bbox/model_size[0] # bounding boxes

    scores = confs * class_probs
    boxes, scores, classes, valid_intersection = tf.image.combined_non_max_suppression(
    ...
    return boxes, scores, classes, valid_intersection


def resize_image(inputs, modelsize): #unuseful funtion, better load tf.image.resize directly
